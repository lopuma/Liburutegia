...
//TODO ✅ ACTIVAR CHECK SI ESTA ACTIVA VENTANA BOOK
    ...
    function asigneColor(data) {
        if(data.delivered === 1 && data.cancelReserved === 0) {
            color = '#A2CDCD';
        } else if(data.delivered === 1 && data.cancelReserved ===1) {
            color = '#FF8B8B';
            ...
            columns: [
                {
                    data: null,
                    render: (data) => {
                        const textColor = asigneColor(data)
                        return `<span style="background-color:${textColor}; cursor:text; font-weight:400;border-radius:3px;padding:0 2px">${fillZeros(data.bookingID)}</span>`
                    }, visible: false
                },
                {
                    ...
                    render: (data) => {
                        const textColor = asigneColor(data)
                        return `<span style="background-color:${textColor}; cursor:text; font-weight:400;border-radius:3px;padding:0 2px">${fillZeros(data.bookID)}</span>`
                    }
                },
                {
                    data: null,
                    render: (data) => {
                        ...
                        const textColor = asigneColor(data);
                        return (`
                                <a style="background-color:${textColor}; margin-left: 1em" class='Linked' href='/workspace/books/info/${data.bookID}'>${title}</a>`
                        );
                    }
                },
                { data: "isbn" },
                {
                    ...
                    render: (data) => {
                        ...
                        return (`
                                <a style="margin-left: .5em" class='Links' href='/workspace/partners/info/${data.partnerID}'>${partnerDni}</a>`
                        );
                    }
                },
                {
