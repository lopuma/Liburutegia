...
const modalNoDeletePartners = document.getElementById('modalNoDeletePartners');
//const modalContent = modalNoDeletePartners.querySelector('.modal-content');
...
async function loadData(data) {
                ...
                render: function (data) {
                    return (
                        `
                            ...
                            <button id="btnDeletePartner" onClick="deletePartner('${data.dni}',${data.partnerID})" type="button" class="btn btn-outline-danger" title="Delete Partner"><i class="fa-solid fa-trash-can"></i></button>
    ...
    $(document).ready(function() {
        setupInputListener('tablePartner_filter', 'btnClearPartners');
    });
}
//TODO ✅ INFO PARTNER
...
async function apiReservActiva(dniPartner, partnerID) {
    const urlDelete = `/api/partners/activeReserve/${dniPartner}`;
    const idPartner = partnerID;
        ...
        .then(async (data) => {
            ...
            if (lengthData >= 1) {
                Swal.fire({
                    icon: "error",
                    title: "Oops...",
                    text: "The member cannot be deleted, because he or she has pending reservations",
                    showCancelButton: true,
                }).then((result) => {
                    if (result.isConfirmed) {
                        if (_URL.indexOf("/workspace/partners/info/") !== -1) {
                            avanzaSlide(1, idPartner)
                        } else {
                            window.location = `/workspace/partners/info/${idPartner}?indice=2&partnerID=${idPartner}`;
                        }
...
async function deletePartner(partnerDNI, partnerID) {
    opcion = 'delete';
    const idPartner = partnerID;
    const dniPartner = partnerDNI;
    Swal.fire({
        title: 'Are you sure?',
        ...
        if (result.isConfirmed) {
            apiReservActiva(dniPartner, idPartner);
        }
    });
}
