...
const inputElementActiveReserved = document.getElementById('searchInput');
let _PARTNERID = "";
let indice = 1;
let _ISBN = "";
...
// Crea un evento de teclado para simular la pulsación de "Enter"
...
if (urlParams.has('indice')) {
    indice = parseInt(urlParams.get('indice'));
}

if (urlParams.has('partnerID')) {
    _PARTNERID = parseInt(urlParams.get('partnerID'));
}
...
async function inforActive(partnerID) {
    ...
    await reloadDataActive(_PARTNERID);
}
...
async function createdTableActive(data) {
    ...
    if (urlParams.has('isbn')) {
        _ISBN = parseInt(urlParams.get('isbn'));
        $('#tablePartnerActive_filter input').val(_ISBN)
        var e = $.Event("keyup", { keyCode: 13 });

        // Dispara el evento de teclado en el input
        $('#tablePartnerActive_filter input').trigger(e);

    }
    $('#tablePartnerActive_filter input').focus();
    $(document).ready(function() {
        setupInputListener('tablePartnerActive_filter', 'btnClearInfoActive');
    });
}
...
async function createdTableHistory(data) {
    ...
    $(document).ready(function() {
        setupInputListener('tablePartnerHistory_filter', 'btnClearInfoHistory');
    });
}
