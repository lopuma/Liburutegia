...
async function apiReservaPartnerActiva(bookID, isbn) {
    const urlDelete = `/api/books/activeReservePartner/${isbn}`;
    const idBook = bookID;
        ...
        .then(async (data) => {
            if (data.results) {
                Swal.fire({
                    icon: "error",
                    title: "Oops...",
                    text: "The book cannot be deleted, because it has pending reservations.",
                    showCancelButton: true,
                }).then((result) => {
                    if (result.isConfirmed) {
                        const idPartner = data.results[0].partnerID;
                        window.location = `/workspace/partners/info/${idPartner}?indice=2&partnerID=${idPartner}&isbn=${isbn}`;
                    }
                });
...
const globalDeleteBook = async (idBook, isbn) => {
    Swal.fire({
        title: "Are you sure?",
        ...
        if (result.isConfirmed) {
            await apiReservaPartnerActiva(idBook, isbn);
        }
    })
}
...
async function notificador(idBook, _isbn, _title) {
    const isbn = _isbn;
    const bookID = idBook;
    const title = _title;
    inputBookIDNT.value = fillZeros(bookID);
    spanBookTITLENT.innerHTML = title
    inputBookTITLENT.value = title
    spanbookISBNNT.innerHTML = isbn
    inputBookISBNNT.value = isbn
}
...
function setupInputListener(filterInput, idButton) {
    let inputElement = $(`#${filterInput} input`);
    const btnClearSearch = document.getElementById(`${idButton}`);

    // Verificar si el input tiene caracteres al cargar la página
    let initialInputLength = inputElement.val().length;
    updateButtonVisibility(initialInputLength);

    // Adjuntar el evento input al input
    ...
    // Adjuntar el evento click al botón para limpiar el input
    btnClearSearch.addEventListener('click', function() {
        inputElement.val(''); // Limpiar el input
        inputElement.focus(); // Enfocar el input
        inputElement.trigger('input'); // Disparar el evento input
        updateButtonVisibility(0); // Actualizar la visibilidad del botón
    });

    function updateButtonVisibility(length) {
        if (length > 0) {
            btnClearSearch.classList.add("visible");
            btnClearSearch.classList.remove("invisible");
        } else {
            btnClearSearch.classList.remove("visible");
            btnClearSearch.classList.add("invisible");
        }
    }
}